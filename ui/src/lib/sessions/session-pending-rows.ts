...
function createOptimisticRowPatches<T>(
  ...
  let nextOrder = 0;
  const notify = (target: PendingRowPresentationTarget) => {
  ...
  return {
    ...
    read(identity: string): { readonly next: T } | undefined {
      ...
      return current && current.sessionId === undefined && current.isCurrent?.() !== false
        ? current
        : undefined;
    },
    ...
    settle(
      ...
      if (target.sessionId === undefined) {
        // Unbound intent is view-only; it never overwrites canonical row fields.
    ...
    applyRow(row: GatewaySessionRow, identity: string | null): GatewaySessionRow {
      ...
      return patch && patch.sessionId !== undefined && row.sessionId === patch.sessionId
        ? fields.write(row, patch.next)
        : row;
    },
...
export function createOptimisticSettingsPatches(host: PendingRowCopyHost) {
  ...
  return {
    ...
    prepare(patch: SessionSettingsPatch, isCurrent: () => boolean) {
      const starts: Array<
        readonly [Pick<typeof thinking, "settle">, ReturnType<typeof thinking.prepare>]
      > = [];
