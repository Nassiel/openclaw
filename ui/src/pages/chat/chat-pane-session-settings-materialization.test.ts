...
it.each(["unbound", "materialized"] as const)(
  ...
  async (lastTarget) => {
    ...
    try {
      ...
      await Promise.allSettled([first, middle, last]);
      await vi.dynamicImportSettled();
    }
  },
);
...
it.each(
  (["absent", "placeholder"] as const).flatMap((initialState) =>
    (["success", "rejection"] as const).map((outcome) => ({ initialState, outcome })),
  ),
)(
  ...
  async ({ initialState, outcome }) => {
    ...
    try {
      ...
      for (const [index, reply] of replies.entries()) {
        ...
        await expect(operation).resolves.toBe(outcome === "success");
      ...
      expect(getPendingChatPickerPatch(pane.state, key, "main")).toBeUndefined();
      if (outcome === "rejection") {
        expect(pane.state.chatError).toContain("Synthetic queued setting 2 rejection");
