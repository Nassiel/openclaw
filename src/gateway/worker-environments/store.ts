...
import { normalizeCredentialHash, required } from "./store-validation.js";
import type {
...
export async function createWorkerEnvironmentStore(
  ...
  const store = {
    ...
    get: (id: string) => read(() => owner.get(required(id, "id"))),
    list: () => read(() => owner.list()),
    listForReconcile: () => read(() => owner.list(true)),
    getCredential: (id: string) => read(() => owner.credential(required(id, "id"))),
    findCredentialByHash: (hash: string) =>
      read(() => owner.credentialByHash(normalizeCredentialHash(hash))),
    getTransferOwner: (id: string) => read(() => owner.transferOwner(required(id, "id"))),
    ...
    async revokeEnvironmentCredential(
      ...
      const environmentId = required(input, "id");
      await mutate(
