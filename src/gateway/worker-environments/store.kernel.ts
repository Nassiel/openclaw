...
import { credentialInsert, revokeCredential, update, upsertCredential } from "./store-mutations.js";
import { find, findCredential, getRequired, json, query } from "./store-row-codec.js";
    ...
    revokeEnvironmentCredential(input: CredentialRevocationInput): void {
      ...
      return write((db) => {
        if (
          ...
          getRequired(db, environmentId).ownerEpoch !== input.expectedOwnerEpoch
        ) {
          throw new Error(`Worker environment ${environmentId} owner epoch changed`);
        }
