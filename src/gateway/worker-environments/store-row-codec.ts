...
export const query = (db: DatabaseSync) => getNodeSqliteKysely<WorkerDb>(db);
...
export function getRequired(db: DatabaseSync, environmentId: string) {
...
export function readWorkerEnvironmentFacts(
  ...
  const environments = ids
    ...
    : listRows(db);
  const credentialQuery = query(db).selectFrom("worker_environment_credentials").selectAll();
  const credentials =
    ids?.length === 0
      ? []
      : executeSqliteQuerySync(
