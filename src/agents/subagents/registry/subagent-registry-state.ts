...
function overlaySubagentRunsSnapshot<T extends SubagentRunReadRecord>(
  ...
  // An older prepared read must neither consume nor clear a newer database's owner intent.
  const state =
    !preparedContext || matchesSubagentCacheContext(cache.state.context, preparedContext)
      ? cache.state
      : {};
  if (shouldReadPersistedSubagentRuns()) {
